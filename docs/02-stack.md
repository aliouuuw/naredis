...
| Database | **PostgreSQL** on **Neon** | Primary source of truth |
...
| Hosting | **Vercel** (app) + **Neon** (DB) + **R2** (files) |
...
| Option | Reason |
|--------|--------|
| Supabase as backend/ORM | Couples product to their client model; we own Postgres on Neon |
